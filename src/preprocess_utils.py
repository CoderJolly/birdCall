...
def mp3_filename(filepath):
    ...
    """
    return get_filename(filepath)[:-4] # [-4] is done to remove .mp3
...
def data_augmentation(filepath):
    ...
    for chunk_species in tqdm(os.listdir(filepath)):
        ...
        save_object_in_dir(df_pitch, f'../4_pitch_change/{chunk_species}_pitch.p') # saving pickle file of each species one by one
        save_object_in_dir(df_time, f'../5_time_change/{chunk_species}_time.p') # saving pickle file of each species one by one        
